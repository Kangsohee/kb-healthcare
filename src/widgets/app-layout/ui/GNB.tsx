...
export function GNB() {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-border bg-bg-default px-6 shadow-sm">
      <div className="flex items-center gap-2">
        <HeartPulse aria-hidden="true" size={32} className="text-primary" />
        <span className="text-2xl font-bold text-primary">KB헬스케어</span>
      </div>
      {accessToken ? (
        ...
        <Link to="/sign-in" aria-label="로그인" className="rounded-lg p-1 text-text-secondary transition-colors hover:bg-bg-subtle hover:text-text-primary">
          <LogIn aria-hidden="true" size={36} />
        </Link>
      )}
    </header>
  )
